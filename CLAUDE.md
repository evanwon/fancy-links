...
### Immediate Commands
```bash
# Test the extension (from src/ directory)
...
This project uses a clean separation between extension code and development files:

### Extension Files (`src/` directory - production code only)
- `src/manifest.json` - Extension manifest defining permissions, browser action, keyboard shortcuts
- `src/background/` - Background scripts for handling clipboard operations and format logic
...
- `src/icons/` - Extension icons in multiple sizes (16px, 32px, 48px, 128px)
- `src/shared/` - Shared utilities and configurations

### Development Files (project root - excluded from builds)
...
- `dist/` - Build output directory (generated by web-ext)
